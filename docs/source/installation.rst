...
You can try out the Streamlit application online at this address `https://lemons.streamlit.app/ <https://lemons.streamlit.app/>`_, generate your own crowd, download the corresponding configuration files, and run the crowd simulation locally.
...
After downloading the repository from `GitHub <https://github.com/Crowd-Mechanics/LEMONS>`__, you can build the ``CrowdMechanics`` library as a shared library in the ``src/mechanical_layer`` directory by running the following commands on Linux or macOS:
